                            ...
                            <div class="card-footer d-flex flex-wrap justify-content-center gap-2 bg-white">
                                ...
                                @else
                                    ...
                                    @if ($escrow->restAmount() <= 0 && $escrow->buyer_id == auth()->user()->id && $hasSellerAndBuyer)
                                        <button class="btn btn--primary confirmationBtn" data-question="@lang('Are you sure to dispatch this escrow?')" data-action="{{ route('user.escrow.dispatch', $escrow->id) }}"><i class="la la-money-check-alt"></i> @lang('Dispatch Payment')</button>
                                    @endif
                                @endif
                            </div>
                        @endif
